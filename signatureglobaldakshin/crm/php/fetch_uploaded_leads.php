...
$duplicateItems = [];
$dupSql = "SELECT mobile FROM uploaded_leads GROUP BY mobile HAVING COUNT(*) > 1 
           UNION 
           SELECT email FROM uploaded_leads GROUP BY email HAVING COUNT(*) > 1";
...
// Fetch Leads
$sql = "SELECT * FROM uploaded_leads";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Delete icon column
    $tableRows .= '<td>
        <a href="php/delete/delete_uploaded_lead.php?id=' . $id . '" class="delete-lead" onclick="return confirm(\'Are you sure you want to delete this lead?\')">
...
$countSql = "SELECT COUNT(*) FROM uploaded_leads";
if (!empty($where)) {
...
function getCount($conn, $status = null) {
    $sql = $status ? "SELECT COUNT(*) FROM uploaded_leads WHERE status = ? AND delete_status != 'deleted'" : "SELECT COUNT(*) FROM uploaded_leads WHERE delete_status != 'deleted' AND bot != 'Bot'";
    $stmt = $conn->prepare($sql);
...
function getUnreadCount($conn) {
    $stmt = $conn->prepare("SELECT COUNT(*) FROM uploaded_leads WHERE read_status = 'unread' AND delete_status != 'deleted' AND bot != 'Bot'");
    $stmt->execute();
    $stmt->store_result();
