...
$employeeNames = [];
try {
    $sql = "SELECT DISTINCT employ_id FROM team_leader_eod ORDER BY employ_id ASC";
...
// Fetch leader's EOD data for date range
$summaryData = [
    'calls' => 0,
    'prospects' => 0,
    'total_team_hot_prospect' => 0,
    'total_team_very_hot_prospect' => 0,
    'meetings' => 0,
    'no_of_ob' => 0,
    'total_days' => 0,
    'total_clients' => 0
...
try {
    // First get all entries for this leader in the date range
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (!isset($dailyAdded[$currentDate])) {
            ...
            $summaryData['total_team_hot_prospect'] += $row['total_team_hot_prospect'];
            $summaryData['total_team_very_hot_prospect'] += $row['total_team_very_hot_prospect'];
            $summaryData['meetings'] += $row['meetings'];
            $summaryData['no_of_ob'] += $row['no_of_ob'];
    ...
    if ($summaryData['total_days'] === 0) {
        // header("Location: leader_eod_detail_report.php?type=error&message=No data found for $fromDate to $toDate dates, Please select the valid dates&employee=$selectedEmployee");
        // exit();
    }
...
<head>
  ...
  <title>Leader EOD Summary - <?= safeOutput($selectedEmployee) ?></title>
...
<body>
  ...
  <div class="form-container">
    <h2>Leader EOD Summary Report</h2>
    <a class="close" href="/leader_eod_report.php">
    ...
    <div class="container">
        <table class="summary-table">
          ...
          <tr><th>Team Hot Prospects</th><td><?= safeOutput($summaryData['total_team_hot_prospect']) ?></td></tr>
          <tr><th>Team Very Hot Prospects</th><td><?= safeOutput($summaryData['total_team_very_hot_prospect']) ?></td></tr>
          <tr><th>Total Meetings</th><td><?= safeOutput($summaryData['meetings']) ?></td></tr>
          <tr><th>Total OB</th><td><?= safeOutput($summaryData['no_of_ob']) ?></td></tr>
  ...
  <script>
      ...
      function exportToPNG() {
          ...
          html2canvas(document.querySelector('.form-container')).then(canvas => {
              ...
              link.download = `Leader-EOD-Report-<?= $selectedEmployee ?>-<?= $fromDate ?>-to-<?= $toDate ?>.png`;
      ...
      function exportToPDF() {
          ...
          html2canvas(document.querySelector('.form-container')).then(canvas => {
              ...
              pdf.save(`Leader-EOD-Report-<?= $selectedEmployee ?>-<?= $fromDate ?>-to-<?= $toDate ?>.pdf`);
      ...
      function exportToExcel() {
          ...
          tempTable.innerHTML = `
              <tr>
                  ...
                  <th>Team Hot Prospects</th>
                  <th>Team V. Hot Prospects</th>
              ...
              <tr>
                  ...
                  <td><?= $summaryData['total_team_hot_prospect'] ?></td>
                  <td><?= $summaryData['total_team_very_hot_prospect'] ?></td>
                  <td><?= $summaryData['meetings'] ?></td>
                  <td><?= $summaryData['no_of_ob'] ?></td>
          ...
          if (<?= !empty($clientData) ? 'true' : 'false' ?>) {
              tempTable.innerHTML += `
                  <tr><td colspan="11"><strong>Client Details</strong></td></tr>
                  <tr>
                      <th>Date</th>
                      <th>Client Name</th>
                      <th>Mobile</th>
          ...
          const link = document.createElement('a');
          link.download = `Leader-EOD-Report-<?= $selectedEmployee ?>-<?= $fromDate ?>-to-<?= $toDate ?>.xls`;
          link.href = url;
          link.click();
      }
  </script>
