...
<head>
    ...
    <title>Admin Dashboard | Leader's EOD Record</title>
            ...
            <div class="details">
                <div class="recentOrders">
                    <div class="cardHeader" style="display: flex; flex-wrap: wrap; gap: 10px; align-items: center;">
                        <h2 id="reportDateTitle" style="font-size: clamp(20px, 5vw, 34px);">Leader's EOD Report (<?= date('d-m-Y', strtotime($reportDate)) ?>)</h2>
                        <div style="display: flex;justify-content: center; align-items: center; flex-wrap: wrap; gap: 5px;">
                            ...
                            <div class="dropdown" style="position: relative; background: transparent; display: inline-block;">
                                ...
                                <div id="reportDropdown" class="dropdown-content" style="display: none; position: absolute; background: #fff; box-shadow: 0 4px 8px rgba(0,0,0,0.1); z-index: 10; border-radius: 5px; min-width: 160px;">
                                    <a href="eod_report.php" style="padding: 10px 16px; display: block; text-decoration: none; color: #333;">View Team Report</a>
                                    <a href="leader_eod_detail_report.php" style="padding: 10px 16px; display: block; text-decoration: none; color: #333;"> View Leader detailed Report</a>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>Name</th>
                                <th>Team Calls</th>
                                <th id="callyzer_td">Team Prospects</th>
                                <th>Team Hot Prospects</th>
                                <th>Team Very Hot Prospects</th>
                                <th id="target_achieved_td">team Meetings</th>
                                <th>No. of OB</th>
                                <th>Project training</th>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function fetchData(date = null) {
            ...
            const url = `php/fetch/fetch_leader_eod.php?date=${selectedDate}`;
            ...
            fetch(url)
                ...
                .then((data) => {
                    ...
                    if (!Array.isArray(data) || data.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="12">No data available for selected date.</td></tr>`;
                        return;
                    }
                    ...
                    data.forEach((row, index) => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td>${row.calls}</td>
                            <td>${row.prospects}</td>
                            <td>${row.total_team_hot_prospect}</td>
                            <td>${row.total_team_very_hot_prospect}</td>
                            <td>${row.meetings}</td>
                            <td>${row.no_of_ob}</td>
                            <td>${row.project_training}</td>
                            <td>${row.client_name}</td>
                            <td>${row.check_out}</td>
                            <td style="display:flex;justify-content: flex-end;">
                                <a class="btn" href="leader_eod_details.php?name=${encodeURIComponent(row.employ_id)}&date=${currentDate}">
                ...
                .catch((err) => {
                    console.error(err);
                    document.getElementById("callyzerTableBody").innerHTML = `<tr><td colspan="12">Error loading data.</td></tr>`;
                });
        }
        ...
        reportDateInput.addEventListener('change', function() {
            ...
            reportDateTitle.textContent = `Leader's EOD Report (${formatDateForDisplay(selectedDate)})`;
    ...
    function confirmDelete(employId, date) {
        if (confirm('Are you sure you want to delete this record?')) {
            window.location.href = `php/delete/delete_leader_eod.php?employ_id=${encodeURIComponent(employId)}&date=${encodeURIComponent(date)}`;
        }
    }
</script>
