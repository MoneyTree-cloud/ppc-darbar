<?php
/**
 * Joyville Vyomora (shapoorjipallonjijoyvillevyomora.com) — Lead form handler
 * Validate → sanitize → POST to MoneyTree CRM + PPC Lead API.
 */
...
$source   = env('API_SOURCE', 'Joyville Vyomora | https://shapoorjipallonjijoyvillevyomora.com');

if ($apiToken === '') {
    fail('Lead service temporarily unavailable. Please call us directly.', 503);
}
...
// Retry without SSL if cert validation fails (common in dev)
...
// PPC Lead API
sendPpcLead($name, $email, $phoneDigits, 'https://shapoorjipallonjijoyvillevyomora.com', 'Pune', 'Joyville Vyomora');

// Always return success to user — API errors are logged server-side
if (is_ajax()) {
    respond_json(true, 'Thank you for your interest in Joyville Vyomora! Our team will contact you within 24 hours.');
}
$_SESSION['form_success'] = 'Thank you for your interest in Joyville Vyomora! Our team will contact you within 24 hours.';
header('Location: index.php#contact');
exit;
